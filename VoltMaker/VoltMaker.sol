...
contract VoltMaker is Ownable {
    ...
    //0x1d18C6fcA6817175FfF59763A36aC03CA9755165 //geyser fund pool
    ...
    function setBridge(address token, address bridge) public onlyOwner {
        // Checks
        require(
            token != volt && token != mtrg && token != bridge,
            "VoltMaker: Invalid bridge"
        );
    ...
    // F6: There is an exploit to add lots of VOLT to the vault, run convert, then remove the VOLT again.
    //     As the size of the swap has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _toVOLT, _convertStep: X1 - X5: OK
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == volt) {
                IERC20(volt).safeTransfer(vault, amount);
                voltOut = amount;
            } else if (token0 == mtrg) {
                voltOut = _toVOLT(mtrg, amount);
            } else {
            ...
            // eg. VOLT - MTRG
            IERC20(volt).safeTransfer(vault, amount0);
            voltOut = _toVOLT(token1, amount1).add(amount0);
        } else if (token1 == volt) {
            // eg. USDT - VOLT
            IERC20(volt).safeTransfer(vault, amount1);
            voltOut = _toVOLT(token0, amount0).add(amount1);
        } else if (token0 == mtrg) {
            // eg. MTRG - USDC
            voltOut = _toVOLT(
                mtrg,
                _swap(token1, mtrg, amount1, address(this)).add(amount0)
            );
        } else if (token1 == mtrg) {
            // eg. USDT - MTRG
            voltOut = _toVOLT(
                mtrg,
                _swap(token0, mtrg, amount0, address(this)).add(amount1)
            );
        } else {
            // eg. MIC - USDT
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    function _toVOLT(address token, uint256 amountIn)
        internal
        returns (uint256 amountOut)
    {
        // X1 - X5: OK
        amountOut = _swap(token, volt, amountIn, vault);
    }
}
